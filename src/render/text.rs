...
fn render_port(out: &mut String, p: &Port) {
    ...
    match &p.partner {
        None => { let _ = writeln!(out, "  partner:    (none)"); }
        Some(partner) => render_partner(out, partner),
    }
    match &p.cable {
        None => { let _ = writeln!(out, "  cable:      (no e-marker / not detected)"); }
...
fn render_partner(out: &mut String, p: &Partner) {
    ...
    let _ = writeln!(out, "  partner:    PD={pd}, altmodes={}",
        p.number_of_alternate_modes.map(|n| n.to_string()).unwrap_or_else(|| "—".into()));
...
fn render_cable(out: &mut String, c: &Cable) {
    ...
    let speed = match c.cable_vdo.as_ref().map(|v| &v.speed) {
        ...
        Some(CableSpeed::Unknown(b)) => { let _ = writeln!(out, "  cable:      {kind}, speed=unknown({b})"); return; }
        None => "—",
    };
    let current = match c.cable_vdo.as_ref().map(|v| &v.max_current) {
        ...
        Some(CableCurrent::Unknown(b)) => { let _ = writeln!(out, "  cable:      {kind}, current=unknown({b})"); return; }
        None => "—",
    };
...
fn render_live(out: &mut String, l: &LiveCharge) {
    ...
    let _ = writeln!(out,
        "  charging:   {w}  ({}, {})",
        v.map(|x| format!("{x:.2} V")).unwrap_or_else(|| "— V".into()),
        i.map(|x| format!("{x:.3} A")).unwrap_or_else(|| "— A".into()));
}
